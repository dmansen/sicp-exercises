...
(define (eval-and exp env)
  (define (eval-and-preds preds)
    (if (no-and-predicates? preds)
        true
        (let ((res (eval (first-and-predicate preds) env)))
          (cond
            ((and res (last-and-predicate? preds)) res)
            ((res (eval-and-preds (rest-and-predicates preds))))
            (else false)))))
  (eval-and-preds (and-predicates exp)))

(define (eval-or exp env)
  (define (eval-or-preds preds)
    (if (no-or-predicates? preds)
        false
        (let ((res (eval (first-or-predicate preds) env)))
          (if res
              res
              (eval-or-preds (rest-or-predicates preds))))))
  (eval-or-preds (or-predicates exp)))
